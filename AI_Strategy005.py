...
class AI_Strategy(IStrategy):
    ...
    # Remove empty API keys from the list
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: Dict) -> DataFrame:
        ...
        if len(self.API_KEYS) == 0:
            self.logger.error("You need at least one API key")
            return dataframe
        ...
        def ask_AI(api_key, model):
            ...
            try:
                self.logger.info("Sending request to AI...")
                ...
                if response.status_code == 200:
                    ...
                    if 'choices' in response_data and len(response_data['choices']) > 0:
                        ...
                        try:
                            ...
                            if start_idx != -1 and end_idx > start_idx:
                                ...
                                return False  # Success - no need to change API key
                            else:
                    ...
                    self.logger.warning(f"Rate limit hit for API key {self.current_api_key_index + 1}")
                    return True  # Need to change API key
                elif response.status_code == 401:
                    self.logger.warning(f"Authentication failed for API key {self.current_api_key_index + 1} (401 Unauthorized), you need to change key")
                    return True  # Need to change API key
                else:
            ...
            return True  # Default to trying next key if any error occurred
        ...
        for index in range(self.current_api_key_index, len(self.API_KEYS)):
            ...
            self.logger.info(f"Using API key: {self.current_api_key_index + 1}/{len(self.API_KEYS)}")
            if not ask_AI(self.API_KEYS[index], self.API_MODEL):  # Success - no need to change API key)
                break

        if self.json_response is None:
            self.logger.warning(f"All {len(self.API_KEYS)} API keys hit rate limits or are not valid, skipping...")
